    ...
    <div className='animate-fade-in'>
        <div className='page-header'>
            ...
            <p className='page-subtitle'>
                {emp?.position} - {emp?.departament || "No Departament"}
            </p>
        </div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
            {cards.map((card, index)=>(
                <div key={index} className='card card-hover p-5 sm:p-6 relative overflow-hidden group flex items-center justify-between'>
                    ...
                    <card.icon className='size-10 p-2.5 rounded-lg bg-slate-100 text-slate-600 group-hover:bg-indigo-50 group-jover:text-indigo-600 transition-colors duration-200'/>
                </div>
            ))}
        </div>
