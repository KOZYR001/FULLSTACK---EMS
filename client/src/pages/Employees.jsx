...
const Employees = () => {
  ...
  const fetchEmployees = useCallback(async () => {
    setLoading(true)
    setEmployess(dummyEmployeeData.filter((emp)=>(selectedDept ? employees.department === selectedDept : emp)))
    setTimeout(() => {
      setLoading(false)
    },1000)
  }, [selectedDept])
  ...
  return (
    <div className='animate-fade-in'>
      ...
      <div className='flex flex-col sm:flex-row gap-3 mb-6'>
        ...
        <select value={sale} onChange={(e)=>setSelectedDept(e.target.value)} className='max-w-40'>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
          {filtered.length === 0 ? (
            ...
            filtered.map(() => <EmployeeCard key={emp.id} employee={emp} onDelete={fetchEmployees} onEdit={(e)=> setEditEmployee(e)}/>)
          )}
        </div>
      )}

      {/* Creaye Employee Modal */}
      {showCreateModal && (
